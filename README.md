...
CliniDraft AI was evaluated across **14 clinical section types** using real patient records. Metrics were computed without any additional API calls, using the agent's existing feedback and correction-memory outputs.

| Metric | Score |
|---|---|
| **Field Extraction Accuracy** | 85.7% across 14 clinical section types |
| **Section Match Rate (SMR)** | 85.7% of sections require zero clinician edits |
| **Avg Normalized Edit Distance** | 0.0254 (lower = fewer corrections needed) |
| **Hallucination Safety Rate** | 100% — zero fabricated clinical values in output |
| **Learning Improvement** | Up to 31.5% NED reduction via doctor-correction memory |

### Benchmark Metrics Dashboard

Visual breakdown of per-section extraction accuracy, NED improvement, and key performance summary across all evaluation runs.

![Benchmark Metrics Dashboard](screenshots/benchmark_chart.png)

### Phase 2: Doctor-Edit Learning Improvement Curve

Shows how the agent's Normalized Edit Distance (NED) drops across learning iterations as it accumulates confirmed correction rules from simulated clinician reviews.

![Learning Improvement Curve](screenshots/improvement_curve.png)

> **Resume Note:** Metrics generated by `benchmark_report.py` — run `python3 benchmark_report.py --save --plot` from the `discharge_summary_agent/` directory to regenerate.
